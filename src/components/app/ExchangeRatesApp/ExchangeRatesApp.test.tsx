...
beforeEach(() => {
  (getCurrencyNames as jest.Mock).mockImplementation(
    () => new Promise((resolve) => resolve(currencyNamesTestData))
  );
  (getCurrenciesExchangeRates as jest.Mock).mockImplementation(
    () => new Promise((resolve) => resolve(exchangeRatesTestData))
  );
});
...
describe('render', () => {
  it('ExchangeRateCard should be rendered in proper amount', async () => {
    ...
    await waitFor(() => screen.findByDisplayValue('1.4748'));
    const exchangeRateCardElements = screen.getAllByRole('article');
...
describe('favorites functionality', () => {
  it('exchangeRateCard should have favorite class', async () => {
    ...
    expect(exchangeRateCardElements[0]).toHaveClass(
      'exchange-rate-card--favorite'
    );
  });
...
describe('exchange date', () => {
  it('should change relative exchange date', async () => {
    ...
    const searchedValue = await screen.findByDisplayValue('1.6666');
